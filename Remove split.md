...
超级管理员可强制网络内全部子站自动继承网络级所有配置。

子站只可以选择“启用”微信支付、支付宝支付，相关支付网关描述的配置。

#### **1.1 子站独立配置**  
网络激活但子站未被强制继承网络级配置，或者，插件未被网络激活，而是子站单独激活插件的情况下。
...
##### **2.1.1 子站不绑定独立域名**  
如果子站不绑定独立域名，而是使用xxx.tao.ooo二级域名，这种情况下，管理员可以在网络级管理面板中强制子站继承网络全站配置，或子站管理员可以选择启用“使用全局配置--继承网络级全局配置”选项而无需配置。
这种情况下，子站的回调地址如何处理才能保证支付/退款等业务正常运行？
