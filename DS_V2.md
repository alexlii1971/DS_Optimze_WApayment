...
特别说明：
1.由于wordpress multisite上支持子站绑定自己独立的域名；比如子站shop.tao.oo可以独立域名如：mydomain.com，这种情况下，插件要能支持生成独立域名的回调地址，以用于满足微信支付、支付宝官方所要求的付款与退款回调地址。
2.当然，如果子站也可以不绑定自己的独立域名。

| **检查项**               | **验证标准**                               | **技术实现**                     |
|--------------------------|-------------------------------------------|---------------------------------|
| 支付绑定独立域名/不绑定独立域名  |提供两种 HTTPS回调域名的解决方法                | 前端正则校验+数据库唯一约束       |
