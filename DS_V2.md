...
### **3. 模块化设计**  
一定要采用模块化的架构设计，避免不同功能及代码之间的过多耦合！
...
由于wordpress multisite上支持子站绑定自己独立的域名；比如子站shop.tao.oo可以独立域名如：mydomain.com，这种情况下，插件要能支持生成独立域名的回调地址，以用于满足微信支付、支付宝官方所要求的付款与退款回调地址。


| **检查项**               | **验证标准**                               | **技术实现**                     |
|--------------------------|-------------------------------------------|---------------------------------|
| 独立域名                |HTTPS回调域名的解决方法                | 前端正则校验+数据库唯一约束       |
| 微信支付配置             | 子站可填写MCH_ID、API密钥，上传证书文件     | 加密存储，PEM格式验证             |
| 支付宝配置               | 子站可设置APP_ID、公私钥                    | RSA密钥对生成指导，字段级加密     |

#### **2.1.1 子站不绑定独立域名**  
如果子站不绑定独立域名，而是使用xxx.tao.ooo二级域名，这种情况下，管理员可以在网络级管理面板中强制子站继承网络全站配置或子站管理员可以选择启用“使用全局配置--继承网络级全局配置”选项；但，这种情况下，子站的回调地址如何处理才能保证支付/退款等业务正常运行？
...
该插件的部署、配置、维护、升级等所有管理功能全部必须在界面化中完成。不能要求用户做linux服务器或wp-cli相关操作及做数据库库表操作！

#### **2.3.1 安装界面** 
由于插件涉及多模式使用，因权限问题、wordpress插件使用惯例及管理，任何安装、部署、配置都在插件界面中完成；不能要求用户做linux服务器或wp-cli相关操作及做数据库库表操作，包括但不限于支付相关功能的证书、SDK、Key等的部署。

#### **2.3.2 管理界面** 
所有有关插件的管理、升级、维护全部在插件中完成，包括但不限于支付相关功能的证书、SDK、Key等。
...
### **6.1 管理员界面**  
充分考虑三种模式下，插件的部署、使用、管理、维护、升级需求，尤其是“配置管理需求“中提到的要求。
...
| 启用及关闭日志           | 启用/关闭日志                              | 启用之后，日志在woocommerce status下查看 |
| 日志导出                 | 可导出审计日志为CSV/PDF                     | 数据导出模块，权限控制            |
| 压力测试报告             | 支持10,000 TPS无丢单                        | JMeter测试计划，结果分析          |

### **7.1 日志分类与权限管理**  
清晰的日志分类；
考虑以上三种插件模式下，不同权限的日志查看方式！

#### **7.1.1支付日志：**  
遵守woocommerce API及管理，显示所有支付日志；
若有可能，显示微信支付、淘宝支付调试、debug日志。

#### **7.1.管理日志：**  
不同模式下，不同权限可以查看的日志类别或分类。
...
## **目录**  
- [1. 全局模式控制体系](#1-全局模式控制体系)  
- [2. 核心模式技术实现](#2-核心模式技术实现)  
- [3. 分账模式设计](#3-分账模式设计)  
- [4. 安全与合规设计](#4-安全与合规设计)  
- [5. 监控与灾备方案](#5-监控与灾备方案)  
- [6. 用户界面设计](#6-用户界面设计)  
...
fetch('https://api.wechat.com/v3/sub-merchants', {
    ...
    body: JSON.stringify({
        ...
        identity_info: { id_number: legalId }
    })
});
```
...
### **4.2 合规性要求**  
- **微信支付服务商规范**：  
  - 分账比例需在签约时明确约定，单笔分账不得超过订单金额的90%。  
  - 特约商户资质必须通过官方接口审核，资质文件需保留5年。  
- **支付宝服务商协议**：  
  - 分账请求需在交易后30天内完成，争议资金可临时冻结。  
...
group_replication_group_name: "payment-cluster"
...
| **测试类型**         | **指标**               | **工具**      | **通过标准**          |
|----------------------|-----------------------|--------------|---------------------|
| 高并发支付           | 10,000 TPS无丢单      | JMeter       | 成功率 ≥ 99.9%       |
| 分账规则计算延迟     | P99延迟 < 500ms       | Locust       | 无规则计算超时       |
| 灾备切换时间         | 主从切换 < 5分钟      | 手动触发     | 业务中断 < 30秒      |
...
| **版本** | **日期**       | **更新摘要**                      |
|----------|---------------|----------------------------------|
| V1.0     | 2023-08-10    | 初版发布                          |
| V5.0     | 2023-09-05    | 整合全模块，补充安全设计、灾备方案  |

### **B. 术语表**  
| **术语**         | **定义**                                                                 |
|------------------|-------------------------------------------------------------------------|
| HSM              | 硬件安全模块，用于保护加密密钥的安全存储与使用                           |
| TPS              | 每秒事务处理量，衡量系统并发处理能力                                     |
...
| 分账规则引擎计算       | 1. 订单金额1000元，设置阶梯分账规则（0-1000元平台抽20%）<br>2. 触发分账 | 平台分账200元，商户到账800元  |  
| 强制退单资金追溯       | 1. 管理员操作强制退单<br>2. 查询资金流水表  | 资金流水显示退单回滚记录       |  
